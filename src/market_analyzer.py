...
"""
    ...
    def generate_market_review(
        ...
        *,
        ...
        Args:
            ...
            theme_result: 涨停题材分析结果（ThemeAnalysisResult 或 None）
        ...
        if not self.analyzer or not self.analyzer.is_available():
            ...
            return self._generate_template_review(overview, news, theme_result=theme_result)

        # 构建 Prompt
        prompt = self._build_review_prompt(overview, news, theme_result=theme_result)
    ...
    def _inject_data_into_review(
        ...
        *,
        ...
        theme_block = self._build_theme_block(theme_result)
        ...
        if theme_block:
            # 在板块主线后注入题材数据
            inject_target = patterns.get("sector_highlights")
            if inject_target:
                review = self._insert_after_section(
                    review,
                    inject_target,
    ...
    def _build_theme_block(self, theme_result: Any) -> str:
        """Build a 涨停题材 analysis data block for injection into the review report."""
        if not theme_result:
            return ""
        ...
        # 题材聚类 (ThemeCluster dataclass)
        themes = getattr(theme_result, "themes", None) or []
        if themes:
            if language == "en":
                lines.append("#### Theme Clusters")
                lines.append("| Theme | Strength | Core Stocks | Continuity |")
                lines.append("|-------|----------|-------------|------------|")
            else:
                lines.append("#### 题材聚类")
                lines.append("| 题材 | 强度 | 核心个股 | 延续性 |")
                lines.append("|------|------|----------|--------|")

            for theme in themes[:8]:
                name = getattr(theme, "theme_name", "-")
                core = getattr(theme, "stocks", [])
                max_con = getattr(theme, "max_consecutive", 0)
                core_list = []
                for c in core[:3]:
                    if isinstance(c, dict):
                        core_list.append(c.get("name", str(c)))
                    else:
                        core_list.append(getattr(c, "name", str(c)))
                core_str = "、".join(core_list) if language == "zh" else ", ".join(core_list)
                stock_count = getattr(theme, "stock_count", len(core))
                lines.append(f"| {name} | {stock_count}只 | {core_str} | {max_con}板 |")

        # 龙头股 (List[Dict] from theme_analyzer)
        dragons = getattr(theme_result, "dragons", None) or []
        if dragons:
            if lines:
                lines.append("")
            if language == "en":
                lines.append("#### Leading Stocks")
                lines.append("| Stock | Theme | Consecutive Limit-Ups | Signal |")
                lines.append("|-------|-------|-------------------------|--------|")
            else:
                lines.append("#### 龙头股")
                lines.append("| 个股 | 所属题材 | 连板数 | 信号 |")
                lines.append("|------|----------|--------|------|")

            for dh in dragons[:6]:
                if isinstance(dh, dict):
                    code = dh.get("code", "-")
                    theme_name = dh.get("theme", dh.get("theme_name", "-"))
                    boards = dh.get("consecutive_boards", dh.get("limit_up_days", "-"))
                    signal = dh.get("signal", dh.get("market_signal", "-"))
                else:
                    code = getattr(dh, "code", getattr(dh, "name", "-"))
                    theme_name = getattr(dh, "theme", getattr(dh, "theme_name", "-"))
                    boards = getattr(
                        dh,
                        "consecutive_boards",
                        getattr(dh, "consecutive_limit_ups", "-"),
                    )
                    signal = getattr(dh, "signal", getattr(dh, "market_signal", "-"))
                lines.append(f"| {code} | {theme_name} | {boards} | {signal} |")

        # 推荐标的
        recommended = getattr(theme_result, "recommended", None) or []
        if recommended:
            if lines:
                lines.append("")
            if language == "en":
                lines.append("#### Recommended Stocks")
                lines.append("| Stock | Reason | Continuity |")
                lines.append("|-------|--------|------------|")
            else:
                lines.append("#### 推荐标的")
                lines.append("| 个股 | 推荐逻辑 | 预期延续 |")
                lines.append("|------|----------|----------|")

            for rec in recommended[:5]:
                if isinstance(rec, dict):
                    code_or_name = rec.get("code", rec.get("name", "-"))
                    reason = self._compact_news_text(rec.get("reason", rec.get("logic", "")), limit=50)
                    continuity = rec.get("continuity", "-")
                else:
                    code_or_name = getattr(rec, "code", getattr(rec, "name", "-"))
                    reason = self._compact_news_text(getattr(rec, "reason", getattr(rec, "logic", "")), limit=50)
                    continuity = getattr(rec, "continuity", "-")
                lines.append(f"| {code_or_name} | {reason} | {continuity} |")
    ...
    def _build_stats_block(self, overview: MarketOverview) -> str:
        ...
        if self._get_review_language() == "en":
            ...
            return "\n".join(
                [
                    f"- **Market Signal**: {light['score']}/100 " f"({light['temperature_label']}, {light['label']})",
...
"""
        ...
        theme_data_block = ""
        if theme_result:
            theme_data_block = self._build_theme_block(theme_result) or ""
            if theme_data_block:
                theme_data_block = (
                    "\n## 涨停题材数据\n（请在【板块主线】章节中自然融入以下题材信息，作为板块主线的补充）\n"
                    + theme_data_block
                    + "\n"
                )
...
"""
    ...
    def _run_daily_review_parts(self) -> MarketLightReviewResult:
        ...
        # 3. 涨停题材分析（仅 A 股）
        theme_result = None
        theme_recommended_codes: List[str] = []
        if self.region == "cn":
            theme_result, theme_recommended_codes = self._run_theme_analysis()

        # 4. 生成复盘报告（融入题材数据）
        report = self.generate_market_review(overview, news, theme_result=theme_result)
        ...
        return MarketLightReviewResult(
            ...
            recommended_codes=theme_recommended_codes,
        )

    def _run_theme_analysis(self) -> tuple[Any, List[str]]:
        """
        执行涨停题材分析（仅 A 股场景调用）

        Returns:
            (theme_result, recommended_stock_codes)
        """
        try:
            ...
            result = theme_analyzer.analyze()

            if not result.report:
                return None, []

            codes = [str(r.get("code")) for r in result.recommended if r.get("code")]
            return result, codes

        except Exception as e:
            logger.warning("[MarketAnalyzer] 涨停题材分析执行失败，跳过: %s", e, exc_info=True)
            return None, []
